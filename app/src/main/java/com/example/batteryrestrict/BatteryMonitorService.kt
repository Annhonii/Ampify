...
class BatteryMonitorService : Service() {
    ...
    // Current discharge session. Charging is a hard boundary: all counters
    // are cleared when charging starts and begin again after unplugging.
    ...
    private fun handleScreenChanged(isOn: Boolean) {
        ...
        // Do not include time spent charging. The charge/unplug transition
        // resets lastScreenSwitchElapsedMs to the exact boundary.
        if (!lastPublishedChargingState && screenIsOn) {
            screenOnAccumMs += delta
        } else if (!lastPublishedChargingState) {
    ...
    private fun resetDischargeSession(
        nowElapsed: Long,
        nowUptime: Long,
        percent: Int,
        clearGraph: Boolean
        ...
        if (clearGraph) {
            // Also clears the persisted Home graph and display summary.
            BatteryStatsStore.resetSession(this, nowElapsed, nowUptime, percent)
        }
    }
    ...
    private fun updateStatsAndNotification() {
        ...
        val chargingChanged = b.isCharging != lastPublishedChargingState
        if (chargingChanged && b.isCharging) {
            // Plugged in: reset the notification and Home graph immediately.
            resetDischargeSession(nowElapsed, nowUptime, b.percent, clearGraph = true)
        } else if (chargingChanged && !b.isCharging) {
            // Unplugged: start counting from this exact battery level/time.
            resetDischargeSession(nowElapsed, nowUptime, b.percent, clearGraph = false)
        } else if (!b.isCharging && lastKnownPercent != -1 && b.percent < lastKnownPercent) {
        ...
        } else if (lastKnownPercent == -1 || b.isCharging) {
            // Keep the baseline at the current level while charging so the
            // charging increase cannot be counted after unplugging.
            lastKnownPercent = b.percent
        }
        lastPublishedChargingState = b.isCharging

        val liveScreenOn = screenOnAccumMs +
            (if (!b.isCharging && screenIsOn) max(0L, nowElapsed - lastScreenSwitchElapsedMs) else 0L)
        val liveScreenOff = screenOffAccumMs +
            (if (!b.isCharging && !screenIsOn) max(0L, nowElapsed - lastScreenSwitchElapsedMs) else 0L)

        liveScreenOnMs = liveScreenOn
        liveScreenOffMs = liveScreenOff

        val totalSessionMs = if (b.isCharging) 0L
        else max(1000L, nowElapsed - sessionStartElapsedMs)
        val awakeMs = if (b.isCharging) 0L
        else max(0L, nowUptime - sessionStartUptimeMs)
        val deepSleepMs = if (b.isCharging) 0L
        else max(0L, totalSessionMs - awakeMs)

        // Rates are extrapolated from what happened since the last plug-in:
        // e.g. 1% lost in 20 min of screen on -> 3%/hr.
        val activeHours = liveScreenOn / 3_600_000.0
        val idleHours = liveScreenOff / 3_600_000.0

        val activeDrainRate = if (!b.isCharging && activeHours > 0.0028 && activePercentDropAccum > 0)
            (activePercentDropAccum / activeHours) else 0.0
        val idleDrainRate = if (!b.isCharging && idleHours > 0.0028 && idlePercentDropAccum > 0)
            (idlePercentDropAccum / idleHours) else 0.0
        ...
        val snapshot = BatteryStatsSnapshot(
            ...
            monitorScreenOnMs = liveScreenOn,
            monitorScreenOffMs = liveScreenOff
        )
    ...
    private fun buildNotification(stats: BatteryStatsSnapshot): Notification {
        ...
        val lines = listOf(
            ...
            "Screen on: ${formatDuration(liveScreenOnMs)} (${formatWhole(activePercentDropAccum)}%)",
            "Screen off: ${formatDuration(liveScreenOffMs)} (${formatWhole(idlePercentDropAccum)}%)",
    ...
    override fun onCreate() {
        ...
        if (b.isCharging) {
            // If the service starts while plugged in, treat that as the same
            // hard reset as a live plug-in event.
            resetDischargeSession(now, nowUptime, b.percent, clearGraph = true)
        }
