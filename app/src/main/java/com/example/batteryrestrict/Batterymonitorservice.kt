...
class BatteryMonitorService : Service() {
    ...
    private fun readBatteryExtras(): BatteryExtras {
        ...
        return BatteryExtras(pct, charging, microAmps / 1000, temp, voltage)
    }
    ...
    private fun updateStatsAndNotification() {
        ...
        if (b.isCharging != lastPublishedChargingState && b.isCharging) {
            ...
            BatteryStatsStore.resetSession(this)
            screenOnAccumMs = 0L
            screenOffAccumMs = 0L
        ...
        val activeDrainRate = if (activeHours > 0.05) (activePercentDropAccum / activeHours).toFloat() else 0f
        val idleDrainRate = if (idleHours > 0.05) (idlePercentDropAccum / idleHours).toFloat() else 0f
        ...
        val snapshot = BatteryStatsSnapshot(
            ...
            awakeMs = awakeMs,
            screenOnMs = currentScreenOn,
            screenOffMs = currentScreenOff,
            currentNowMa = b.currentNowMa,
            tempCelsius = b.tempTenths / 10f,
            voltageMv = b.voltageMv
        )

        BatteryStatsStore.saveStats(this, snapshot)
        BatteryStatsStore.appendHistoryPoint(this, b.percent, sessionStartElapsedMs)
    ...
    private fun buildNotification(stats: BatteryStatsSnapshot): Notification {
        ...
        val title = if (stats.isCharging) {
            "Charging: ${stats.percent}% (${abs(stats.currentNowMa)} mA)"
        } else {
